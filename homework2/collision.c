...
float cross_product(line_t line, point_t pt) {
    float diff_line_x = line.end_pt.x - line.st_pt.x;
    float diff_line_y = line.end_pt.y - line.st_pt.y;
...
int chekcPolyIntersection(object_t poly0, object_t poly1) {
    ...
    int polygon1_size = poly0.num_pts;
...
int chekcLineIntersection(line_t line0, line_t line1) {
    float orientation1 = cross_product(line0, line1.st_pt);
    float orientation2 = cross_product(line0, line1.end_pt);
    float orientation3 = cross_product(line1, line0.st_pt);
    float orientation4 = cross_product(line1, line0.end_pt);
...
int collision(vector_t poly0, vector_t poly1) {
    ...
    object_t polygons[2];
    object_t obj0, obj1 = {0};

    int poly0_size = poly0.size;
    obj0.num_pts = poly0_size;
    for (int i = 0; i < poly0_size; i++) {
        obj0.pt_array[i][0] = poly0.buffer[i][0];
        obj0.pt_array[i][1] = poly0.buffer[i][1];
    }
    polygons[0] = obj0;
    ...
    for (int i = 0; i < poly1_size; i++) {
        obj1.pt_array[i][0] = poly1.buffer[i][0];
        obj1.pt_array[i][1] = poly1.buffer[i][1];
    }
    polygons[1] = obj1;
