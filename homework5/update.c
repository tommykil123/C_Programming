...
void refreshSimulation(state_t *state) {
    srand(0);
    int initial_runner_idx = state->tune_values.initial_runner_idx;
    ...
    if (state->selected.selected_init_idx) {
        ...
        while (1) {
            ...
            if (!(MAP[idx_y * MAP_W + idx_x] == 'X')) {
                state->tune_values.initial_runner_idx = initial_runner_idx;
                break;
            }
        }
    } else if (state->selected.selected_delay) {
        state->tune_values.delay_every += (int)sign;
        if (state->tune_values.delay_every < 1) {
            state->tune_values.delay_every = 1;
        }
    } else if (state->selected.selected_2goal_mag) {
        state->tune_values.to_goal_magnitude *= pow(2.0, sign);
    } else if (state->selected.selected_2goal_pow) {
        state->tune_values.to_goal_power += sign;
        if (state->tune_values.to_goal_power > 3.0) {
            state->tune_values.to_goal_power = 3.0;
        } else if (state->tune_values.to_goal_power < -3.0) {
            state->tune_values.to_goal_power = -3.0;
        }
    } else if (state->selected.selected_avoid_mag) {
        state->tune_values.avoid_obs_magnitude *= pow(2.0, sign);
    } else if (state->selected.selected_avoid_pow) {
        state->tune_values.avoid_obs_power += sign;
        if (state->tune_values.avoid_obs_power > 3.0) {
            state->tune_values.avoid_obs_power = 3.0;
        } else if (state->tune_values.avoid_obs_power < -3.0) {
            state->tune_values.avoid_obs_power = -3.0;
        }
    } else if (state->selected.selected_max_vel) {
        state->tune_values.max_velocity += sign;
        if (state->tune_values.max_velocity < 1.0) {
            state->tune_values.max_velocity = 1.0;
        } else if (state->tune_values.max_velocity > 12.0) {
            state->tune_values.max_velocity = 12.0;
        }
    }
}
