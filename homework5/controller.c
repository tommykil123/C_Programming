...
void potential_field(state_t *state) {
    tune_values_t tune_values = state->tune_values;
    ...
    double to_goal_dist = distance(state->chaser.x, state->chaser.y,
                                   state->runner.x, state->runner.y);
    double fx_runner = to_goal.x * tune_values.to_goal_magnitude *
                       pow(to_goal_dist, tune_values.to_goal_power);
    double fy_runner = to_goal.y * tune_values.to_goal_magnitude *
                       pow(to_goal_dist, tune_values.to_goal_power);
    fx += fx_runner;
    fy += fy_runner;
    ...
    for (int i = 0; i < MAP_W; i++) {
        for (int j = 0; j < MAP_H; j++) {
            if (MAP[j * MAP_W + i] == 'X') {
                ...
                double fx_wall = to_chaser.x * tune_values.avoid_obs_magnitude *
                                 pow(to_obs_dist, tune_values.avoid_obs_power);
                double fy_wall = to_chaser.y * tune_values.avoid_obs_magnitude *
                                 pow(to_obs_dist, tune_values.avoid_obs_power);
    ...
    state->chaser.vel = fmin(tune_values.max_velocity, state->chaser.vel + 2.0);
}
